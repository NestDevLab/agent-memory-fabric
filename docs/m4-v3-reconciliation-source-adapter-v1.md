...
- SQLite: `driver`, `databasePath`, and bounded `pageSize`. The database must be an owner-only, regular, non-symlink file no larger than 16 TiB. The adapter pins its inode before opening it read-only and retains that descriptor until collection closes.
- PostgreSQL: `driver`, credentials and endpoint fields, bounded `pageSize` and four bounded timeout fields, plus explicit `sslMode` (`disable`, `require`, or `verify-full`). `verify-full` additionally requires an owner-only CA file.
