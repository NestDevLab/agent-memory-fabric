...
| B2.3 | Add or correct source tests for catalogue-resolved wildcard reads, exact write grants, revoked principals, idempotency, revision conflict, audit, and tombstones | B1.4, B2.2 | im-developer / AMF lane | Codex/gpt-5.6-sol/high | DONE | Server persists the provisioned tool list, filters `tools/list` in caller order, denies unlisted calls before dispatch, and denies interactive actors missing a persisted list. Legacy `allowedScopes: ['*']` is now read-only fallback and cannot authorize proposals. Real-server negatives cover generic MCP extras, missing-tool-list actors, and the legacy proposal wildcard; provisioner, server, and full suites pass. | 2026-08-12 - no registry, handoff, token, client, or runtime state was changed; an independent exact-head review is required before B2.4 |
...
| AMF source behavior | Focused ACL/MCP/document tests and complete `npm test` | im-developer | PASS | `node --test scripts/test-interactive-mcp.mjs scripts/test-recall-consumer-provisioning.mjs scripts/test-fabric-server.mjs`: 57 pass, 0 fail; complete `npm test` exited 0 with no failure markers after the server-authoritative remediation |
...
| Grant enforcement | Positive and negative tool tests for wildcard-read, exact-write, unknown, revoked, conflict, and audit behavior | im-developer | PASS pending independent review | The real-server ACL test proves the exact nine-tool list, rejects `context_search`, `list_scopes`, and `gateway_health`, fails closed for an unprovisioned `client:mcp:*`, and denies legacy wildcard proposals; existing suite retains conflict, idempotency, audit, tombstone, and revocation coverage |
...
- In progress: independent exact-head review of the completed B2.3 server-authoritative ACL remediation and real-server negative coverage.
...
- Next safe action: capture the validated source commit in the local Syncwheel draft and obtain a new independent exact-head review before any B2.4 publication.
