...
| B2.2 | Rebase or reconstruct the AMF feature change only onto an approved post-#158 current base | B2.1, B3.3 | im-developer / AMF lane | Codex/gpt-5.6-sol/high | IN_PROGRESS | B2.1 is complete with a NOT READY verdict. Syncwheel created retained draft `amf-unified-mcp-acl` at post-#158 base `540388a`, but correctly refused direct capture because the three local commits are siblings of that base. Declare their exact range, then rebuild it through plumbing; do not use manual reset/rebase. | 2026-08-12 - fix both server-authoritative ACL blockers on the reconstructed stack before re-review |
...
| B3.3 | Rebuild AMF `main-integration` from the verified post-#158 `origin/main` using Syncwheel plumbing and validate convergence | B3.2 | im-operator / Syncwheel lane | Codex/gpt-5.6-terra/high | IN_PROGRESS | Recovery ref `refs/recovery/amf-b3-rebuild-input-20260812` -> `8c43065908389f02c97c41accdda07889c046127` retained. Post-#158 dry run identified unassigned sibling commits; the draft `syncwheel/draft/amf-unified-mcp-acl` exists at `540388a` and direct capture correctly failed rather than replaying them from the wrong base. | 2026-08-12 - B2.2 declaration/rebuild is the required plumbing prerequisite; no guard bypass, worktree/stash/branch deletion, or remote push is in scope |
