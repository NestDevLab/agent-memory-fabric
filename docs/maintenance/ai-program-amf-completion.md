...
| B4.2 | Recover unique adapter intent onto a current-base AMF runtime-adapter lane without reviving obsolete behavior | B4.1 | im-developer / adapter lane | Codex/gpt-5.6-sol/high | IN_PROGRESS | Current-base tests, source review, and named retained recovery refs for ambiguous work | 2026-08-12 - recovery begins with per-branch patch classification and a named non-destructive recovery envelope; session-identity regressions and MGT-0113 fixes require separate evidence |
...
- In progress: B4.2 is classifying retained adapter patches onto a clean current-base lane. PR #159 remains open pending checks/review and a separate merge decision.
...
- Next safe action: capture/push this PR evidence update to #159, then collect CI/review before any merge action.
