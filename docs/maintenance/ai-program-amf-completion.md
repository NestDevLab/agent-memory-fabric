...
| B2.2 | Rebase or reconstruct the AMF feature change only onto an approved post-#158 current base | B2.1, B3.3 | im-developer / AMF lane | Codex/gpt-5.6-sol/high | DONE | Retained stack and `main-integration` reconstructed at `f2df4eb882db271bc5c9a547a08d66ee22350e52` on `540388a`. `git range-diff 9592697..f4b05a9 origin/main..f2df4eb` is five 1:1 matches; `npm test` exited 0; `git diff --check` passed. | 2026-08-12 - draft remains NOT READY until the two B2.1 blockers and real-server negatives are fixed |
| B2.3 | Add or correct source tests for catalogue-resolved wildcard reads, exact write grants, revoked principals, idempotency, revision conflict, audit, and tombstones | B1.4, B2.2 | im-developer / AMF lane | Codex/gpt-5.6-sol/high | DONE | Server constrains every `client:mcp:*` actor to the fixed, canonical Codex/Claude nine-tool contract; absent, partial, injected, or unknown-principal lists receive no tools. The provisioner persists that contract and adds a rollback-capable `--migrate` path that replaces exactly one established principal, policy actor, and matching context key while issuing a new handoff. Legacy `allowedScopes: ['*']` cannot authorize proposals. | 2026-08-12 - independent reviewer READY at `2f4f89c5c9ff914d8743effdbada8e717bad8800`; focused suite 58 pass, 0 fail; complete `npm test` exited 0 |
| B2.4 | Deliver the reviewed AMF source through its approved PR and prove post-merge absorption | B2.3 | im-operator / AMF delivery | Codex/gpt-5.6-terra/high | IN_PROGRESS | PR #159 — `feat(amf): enforce unified MCP ACL and retire Mem0 adapter` is open from `pr/amf-unified-mcp-acl` to `main`, initially at `1e2f748055ef9e2e71ffaf92521dad781ce28643`. GitHub reports a clean merge state, with no checks or reviews reported at creation. | 2026-08-12 - G4 used only for promotion/push/PR creation; wait for checks and a separate merge decision, then prove absorption |
...
| B4.2 | Recover unique adapter intent onto a current-base AMF runtime-adapter lane without reviving obsolete behavior | B4.1 | im-developer / adapter lane | Codex/gpt-5.6-sol/high | IN_PROGRESS | Named recovery refs now preserve `main` plus the six unique source candidates; `git cherry origin/main` proves all other legacy adapter/runtime/M4 stack patches are equivalent. A branch/worktree rotation is required before a clean canonical integration lane can be materialized. | 2026-08-12 - stop before broad `reconcile --apply`: retained `main-integration` is held by a clean legacy worktree; G11 or an equivalent exact non-destructive rotation decision must name its disposition |
...
| AMF source behavior | Focused ACL/MCP/document tests and complete `npm test` | im-developer | PASS pending independent review | `node --test scripts/test-interactive-mcp.mjs scripts/test-recall-consumer-provisioning.mjs scripts/test-fabric-server.mjs`: 58 pass, 0 fail; complete `npm test` exited 0 after the final remediation. |
...
| Grant enforcement | Positive and negative tool tests for wildcard-read, exact-write, unknown, revoked, conflict, and audit behavior | im-developer | PASS pending independent review | The real-server ACL test covers the exact nine tools, injected extra/missing/unknown `client:mcp:*` lists, generic extras, legacy wildcard proposals, and existing conflict/idempotency/audit/tombstone/revocation cases. |
...
| PR delivery | Exact SHA, current base, independent review, CI, merge/retention decision, and absorption proof | im-operator | IN PROGRESS for AMF | PR #159 is open and `CLEAN`; no GitHub checks/reviews are reported yet. Source `2f4f89c` was independently READY; CI and a separate merge/absorption decision remain. PR #49 remains excluded. |
...
| R9 | The local bridge's nine-tool contract is bypassable through Fabric's generic MCP endpoint, and legacy wildcard proposal authorization survives | A bearer could reach tools or proposal scopes outside the documented MCP contract | im-developer / AMF lane | Implemented and pending final independent exact-head review: fixed contract rejects generic/injected/missing/unknown MCP tool lists, and wildcard proposal fallback is denied | Do not publish until the final review is READY |
...
| R11 | The adapter primary is a protected divergent `main`, while the canonical `main-integration` branch is held by a retained legacy worktree | A broad reconcile or informal checkout change can rewrite historical integration state or violate the no-cleanup gate | im-coordinator | Named B4.2 recovery refs protect every unique candidate; source recovery starts only from a decision that names the integration rotation and retained worktree disposition | Request the exact non-destructive rotation / G11 decision before B4.2 source changes |

## Handoff snapshot

- As of: 2026-08-12T17:05:00+00:00
- Authoritative item state: `MGT-0029` is `in-progress`; its append-only log records this program.
- Revisions and dirty residue: AMF `main-integration` and draft `syncwheel/draft/amf-unified-mcp-acl` are clean at reconstructed `f2df4eb`; recovery refs preserve original commits and pre-rebuild branch heads. The local `amf-completion` manifest overlay is ignored by Git but selected explicitly. Other repository counts remain in the B0.3 inventory and are untouched.
- Completed evidence: B0.1-B0.3, B1.1-B1.4, B2.1 (NOT READY), B2.2, and B3.1-B3.3. PR #158 merged as `540388a`; PR #49 is open/dirty and excluded; health is degraded only by the retained Claude dead letters.
- In progress: B4.2 has classified the only six non-equivalent adapter candidates and created named recovery refs; it is paused before the required integration branch/worktree rotation. PR #159 remains open pending checks/review and a separate merge decision.
- Blocked: B4.2 requires an exact non-destructive rotation decision for the retained adapter `main-integration` worktree/branch. G8 dead-letter movement and G11 cleanup also remain intentionally open until exact targets exist.
...
- Next safe action: capture/push this evidence update to #159, then obtain the exact adapter integration-rotation decision before source recovery.
