...
| B2.3 | Add or correct source tests for catalogue-resolved wildcard reads, exact write grants, revoked principals, idempotency, revision conflict, audit, and tombstones | B1.4, B2.2 | im-developer / AMF lane | Codex/gpt-5.6-sol/high | IN_PROGRESS | Implement server-authoritative nine-tool allowlisting and legacy proposal-wildcard rejection/migration; add real-server negative tests for generic MCP extras, list/status extras, and wildcard legacy actors before re-review. | 2026-08-12 - retain existing unknown scope/vault and write-wildcard negatives; do not touch live registry/runtime |
...
- As of: 2026-08-12T16:21:00+00:00
...
- In progress: B2.3 server-authoritative ACL remediation and real-server negative coverage.
...
- Next safe action: Implement the two B2.1 blockers on the reconstructed draft, add server-level negative tests, rerun the suite, and obtain a new independent exact-head review.
