#!/usr/bin/env node
import { provisionDocumentClient } from '../src/operator/recall-consumer-provisioning.mjs';
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (option === '--scope') {
      ...
      output.scopes ||= []; output.scopes.push(value); index += 1; continue;
    }
  ...
  if ([...VALUE_OPTIONS.values()].some(key => !Object.hasOwn(output, key)) || !output.scopes?.length) {
...
function safeError(error) {
  const code = String(error?.message || 'document_client_provisioning_failed');
  return /^[a-z0-9_]{1,128}$/.test(code) ? code : 'document_client_provisioning_failed';
}

try {
  process.stdout.write(`${JSON.stringify(provisionDocumentClient(parseArguments(process.argv.slice(2))))}\n`);
} catch (error) {
