#!/usr/bin/env node
import { provisionRecallConsumer } from '../src/operator/recall-consumer-provisioning.mjs';
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (option === '--scope') {
      ...
      output.additionalScopes ||= []; output.additionalScopes.push(value); index += 1; continue;
    }
...
function safeError(error) {
  const code = String(error?.message || 'recall_consumer_provisioning_failed');
  return /^[a-z0-9_]{1,128}$/.test(code) ? code : 'recall_consumer_provisioning_failed';
}

try {
  process.stdout.write(`${JSON.stringify(provisionRecallConsumer(parseArguments(process.argv.slice(2))))}\n`);
} catch (error) {
