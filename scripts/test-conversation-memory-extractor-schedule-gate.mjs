...
test('quality evaluation uses bounded injected dependencies, aggregates only, and never proposes or writes state', async t => {
  ...
  assert.equal(result.sample.requested, 3); assert.equal(result.sample.triageRejected, 1); assert.equal(result.sample.modelFailed, 1); assert.equal(result.sample.candidateClaims, 2); assert.equal(result.sample.invalidOrUnsafeClaims, 1); assert.equal(result.sample.duplicateClaims, 0); assert.equal(result.sample.wouldProposeClaims, 1);
  ...
  assert.equal(seen.some(value => value.includes('/proposals') || value.includes('/memory/search')), false); assert.equal(fs.existsSync(config.stateFile), false);
});
