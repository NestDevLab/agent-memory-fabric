...
test('the bridge exposes the complete common MCP surface and forwards only operation grants', async () => {
  ...
  try {
    ...
    const proposal = await bridge.handleRpc({ jsonrpc: '2.0', id: 5, method: 'tools/call', params: { name: 'memory_propose', arguments: { scope: 'person:joseph', text: 'private candidate', metadata: { source: 'test' }, idempotencyKey: 'proposal-1' } } }); assert.equal(proposal.error, undefined);
    ...
    const deniedProposal = await bridge.handleRpc({ jsonrpc: '2.0', id: 6, method: 'tools/call', params: { name: 'memory_propose', arguments: { scope: 'domain:synthetic', text: 'x', idempotencyKey: 'proposal-2' } } }); assert.equal(deniedProposal.error.code, -32602);
