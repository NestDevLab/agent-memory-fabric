...
function value(label,{excluded=false}={}) {
  ...
  const logicalInput={canonicalSenderIdentity:'synthetic-sender',senderTag:sender,conversationTag:conversation,direction,nativePlatform:'synthetic-platform',nativeConversationId:'synthetic-conversation',nativeMessageId:`native-${label}`}; const logical=deriveLogicalMessageIds(logicalInput,KEYS.logicalMessageKeys);
  const raw=Buffer.from(`native-raw-${label}`); const eventId=deriveEventIdV2({sourceKind:'codex',observationClass:'native',rawBytes:raw}); const sessionId=deriveSessionIdV2({sourceKind:'codex',conversationTag:conversation}); const normalized={role:excluded?'system':'user',contentType:excluded?'structured':'text',value:excluded?{ignored:true}:`visible ${label}`};
  const event={schema:'amf.raw-event/v2',eventId,sessionId,occurredAt:'2026-07-21T12:00:01.000000000Z',source:{runtime:'codex',subtype:'message'},logical:logicalInput,normalized,raw:{encoding:'base64',line:raw.toString('base64'),lineEnding:'lf'}};
  const projection={schema:'amf.raw-event-projection/v2',eventId,sessionId,logicalMessageId:logical.logicalMessageId,logicalMessageAliases:logical.aliases,derivationVersion:'amf-logical-message/v1',keyVersion:logical.keyVersion,sourceKind:'codex',observationClass:'native',direction,conversationKind:'dm',contextTags:{actor:[tag('actor','synthetic-actor')],sender:[sender],conversation:[conversation],room:[tag('room','synthetic-room')]},subtype:'message',occurredAt:event.occurredAt,editedAt:null,nativeRevision:1,sourceSequence:1,authoritativeDeletion:false,role:normalized.role,contentType:normalized.contentType,contentParts:1,hasContent:true,normalizationVersion:'amf-observation-normalization/v1',normalizedPayloadDigest:normalizedObservationDigest({event},DIGEST_KEY)};
