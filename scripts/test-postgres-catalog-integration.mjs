...
test('real PostgreSQL catalog integration in an explicitly isolated test database', { skip: !enabled }, async () => {
  ...
  try {
    ...
    const receipt = { kind: 'decision', proposalId: results[0].record.id, proposalScope: `domain:${suffix}`, decisionId: `decision-${suffix}`, status: 'review_required', decisionDigest: crypto.createHash('sha256').update(`decision-${suffix}`).digest('hex'), proposalDigest: crypto.createHash('sha256').update(`proposal-${suffix}`).digest('hex'), policyDigest: crypto.createHash('sha256').update(`policy-${suffix}`).digest('hex'), timestamp: clock().toISOString() };
