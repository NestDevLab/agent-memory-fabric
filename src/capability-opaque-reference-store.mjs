...
function postgresSsl(value) {
  ...
  const input = record(value); const keys = input && Object.keys(input).sort().join('\0');
  if (!input || !['rejectUnauthorized', 'ca\0rejectUnauthorized'].includes(keys) || typeof input.rejectUnauthorized !== 'boolean' || (Object.hasOwn(input, 'ca') && (input.rejectUnauthorized !== true || typeof input.ca !== 'string' || input.ca.length < 1 || input.ca.length > 1048576 || input.ca.includes('\0')))) fail(INVALID_INPUT);
  return frozen(Object.hasOwn(input, 'ca') ? { rejectUnauthorized: input.rejectUnauthorized, ca: input.ca } : { rejectUnauthorized: input.rejectUnauthorized });
}
export class PostgresOpaqueReferenceStore extends BaseStore {
  constructor({ pool, poolFactory, connectionString, ssl = { rejectUnauthorized: true }, ...options } = {}) { super(options); const secure = postgresSsl(ssl); if (!pool && (typeof connectionString !== 'string' || connectionString.length < 1 || connectionString.length > 8192 || /[\0\r\n]/.test(connectionString))) fail(INVALID_INPUT); const poolOptions = { connectionString, ssl: secure }; this.pool = pool || (poolFactory ? poolFactory(poolOptions) : new Pool(poolOptions)); if (!this.pool || typeof this.pool.query !== 'function') fail(INVALID_INPUT); this.initialized = null; }
