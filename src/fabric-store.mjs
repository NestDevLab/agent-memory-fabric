...
export class FileRawStore extends RawStoreBase {
  ...
  async commit(prepared) {
    const target = this.blobPath(prepared.contentId);
    fs.mkdirSync(path.dirname(target), { recursive: true, mode: 0o700 });
    let created = false;
    try {
      fs.writeFileSync(target, JSON.stringify(prepared.envelope), { encoding: 'utf8', flag: 'wx', mode: 0o600 });
      created = true;
    } catch (error) {
      if (error?.code !== 'EEXIST') throw error;
    }
    return { contentId: prepared.contentId, storageRef: path.relative(this.rootPath, target), byteLength: prepared.plaintextBytes, created };
  }
  ...
  async get(contentId) {
    let envelope;
    try {
      envelope = JSON.parse(fs.readFileSync(this.blobPath(contentId), 'utf8'));
    } catch (error) {
  ...
  async remove(contentId) { fs.rmSync(this.blobPath(contentId), { force: true }); }
}
...
export class FabricStore {
  constructor({ rawStore, catalog, clock = () => new Date(), idFactory = () => crypto.randomUUID() }) {
  ...
  async _catalogOperation(operation) {
    try {
      ...
      if (error?.message === 'catalog_unavailable' || error?.message === 'catalog_schema_version_unsupported') throw error;
...
export function createFabricStoreFromEnv({ rootPath = process.cwd(), env = process.env, postgresPoolFactory } = {}) {
  ...
  return new FabricStore({ rawStore, catalog });
}
