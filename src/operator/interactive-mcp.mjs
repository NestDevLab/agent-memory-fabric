...
function definitions() {
  return {
    ...
    memory_propose: { name: 'memory_propose', description: 'Queue an authorized memory proposal for curation; it never applies a memory directly.', inputSchema: { type: 'object', additionalProperties: false, properties: { record: { type: 'object' }, rationale: { type: 'string', minLength: 1, maxLength: 4096 }, expectedRevision: { type: ['integer', 'null'], minimum: 0 }, idempotencyKey: { type: 'string', minLength: 1, maxLength: 192 } }, required: ['record', 'rationale', 'idempotencyKey'] } },
...
export function createInteractiveMcpBridge({ handoff, fetchImpl = globalThis.fetch, clock = () => Date.now(), randomBytes = crypto.randomBytes } = {}) {
  ...
  async function callTool(name, args) {
    ...
    if (name === 'memory_propose') { requireOnly(args, new Set(['record', 'rationale', 'expectedRevision', 'idempotencyKey'])); if (!object(args.record) || !handoff.proposeScopes.includes(String(args.record?.scope?.id || '')) || typeof args.rationale !== 'string' || !args.rationale.trim() || typeof args.idempotencyKey !== 'string' || !args.idempotencyKey || (args.expectedRevision !== undefined && args.expectedRevision !== null && (!Number.isInteger(args.expectedRevision) || args.expectedRevision < 0))) fail('interactive_mcp_tool_input_invalid'); return request(new URL('v2/memory/proposals', handoff.endpoint), { method: 'POST', headers: headers({ 'idempotency-key': args.idempotencyKey }), body: JSON.stringify({ record: args.record, rationale: args.rationale, expectedRevision: args.expectedRevision ?? null }) }); }
