...
export function validatePamRuntimePrivateDir({
  ...
  if (typeof directory !== 'string' || !directory.trim() || expectedUid === null || !Number.isSafeInteger(expectedUid) || expectedUid < 0) throw invalid('configuration');
  const absolute = path.resolve(directory);
  let directoryFd;
  try {
    const before = fs.lstatSync(absolute);
    if (!before.isDirectory() || before.isSymbolicLink() || before.uid !== expectedUid || !exactMode(before, 0o700)) throw invalid('parent');
    ...
    if (!opened.isDirectory() || opened.uid !== expectedUid || !exactMode(opened, 0o700) || fs.realpathSync(handle) !== absolute) throw invalid('parent');
    ...
    for (const [variable, filename] of Object.entries(PAM_RUNTIME_PRIVATE_FILES)) {
      ...
      try {
        ...
        if (!stat.isFile() || stat.nlink !== 1 || stat.uid !== expectedUid || !exactMode(stat, 0o600) || stat.size < 1 || stat.size > 8 * 1024 * 1024) throw invalid('file');
    ...
    return { ok: true, uid: expectedUid, files: Object.keys(PAM_RUNTIME_PRIVATE_FILES).length };
  } catch (error) {
    if (error?.code === 'pam_runtime_private_dir_invalid') throw error;
